...
const DataProduct = () => {
  ...
  const productItems = isCreatePage ? createOrderState?.items : detailOrderState?.items;
  ...
  const onDeleteProductHandler = (deleteIndex, deleteId) => {
    confirm().then(async () => {
      deleteProductHandler(deleteIndex, deleteId);
    });
  };

  const onClickSyncOrderHandler = (row) => {
    confirm().then(async () => {
      syncProductOrderHandler(row?.id);
    });
  };
